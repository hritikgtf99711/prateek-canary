...
const [isMobile, setIsMobile] = useState(false);
  ...
  return (
    ...
    <section
      id="location"
      style={{
        backgroundImage: `linear-gradient(to top, rgba(0, 0, 0, 0.71) 0%, rgba(0, 0, 0, 0.00) 75%),linear-gradient(to bottom, rgba(0, 0, 0, 0.71) 0%, rgba(0, 0, 0, 0.00) 39%),url(${
          import.meta.env.VITE_BASE_URL
        }/assets/home/locationMap/location.jpg)`,
      }}
      className="w-[100vw] z-[99]  relative overflow-hidden bg-cover h-[85vh] xl:h-[87vh] h-screen  bg-center bg-no-repeat "
    >
      <h2
      ...
      <div className="hidden xl:flex flex-col flex-wrap items-center  w-[100%] mt-[50px] xl:mt-[80px] justify-center">
        <div className="flex justify-between xl:w-[64%] 2xl:w-[66%]"> {data.map((item, idx) => (
                  <div key={item.name} className="flex justify-center w-[180px]  rounded-[20px] xl:p-[20px] h-[150px] !text-white items-center flex-col bg-[#295245e3]">
                    <img  loading="lazy" 
  decoding="async"
                      src={ item.img}
                      alt={item.name}
      ...
      {/* for mobile only */}
      <div className="2xl:top-[50%] xl:hidden block right-[10%] xl:w-auto w-[80%] top-[57%] lg:right-[60px] xl:right-[20px] absolute translate-y-[-50%] ">
